 ...
 *
 * Wichtig: hier wird nichts erfunden. Die Bausteine kommen ausschliesslich
 * aus bestaetigten Aussagen des Menschen. Was fehlt, wird als Luecke
...
export interface GeneratedCv {
  sections: CvSection[];
  /** Was fehlt, damit der Mensch es ergaenzen kann. */
  gaps: string[];
...
export function buildCv(
  ...
  if (skills.length > 0) {
    sections.push({
      heading: locale === "en" ? "Skills" : "Faehigkeiten",
      entries: skills.map((e) => ({ text: e.statement, evidenceIds: [e.id] })),
    });
  }

  // Luecken gegen die Muss-Anforderungen der Stelle benennen.
...
export function coverLetterAdvisable(job: Job): { advisable: boolean; reason: string } {
  ...
  if (asksForIt) {
    return { advisable: true, reason: "Die Anzeige verlangt ausdruecklich ein Anschreiben." };
  }
  if (job.applyMethod === "portal") {
    return {
      advisable: false,
      reason:
        "Die Bewerbung laeuft ueber ein Portal und die Anzeige verlangt kein Anschreiben. " +
        "Eine kurze, praezise Antwort in den Portalfeldern wirkt hier meist besser.",
    };
  }
  return {
    advisable: false,
...
export function buildApplicationEmail(
  ...
  const confirmed = evidence.filter(isConfirmedFact);
  // Die zwei staerksten Belege: bestaetigt, mit hoher eigener Sicherheit.
