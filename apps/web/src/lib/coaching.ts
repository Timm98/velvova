...
function buildQuestions(
  ...
  questions.push({
    ...
    origin: "Standardfrage in nahezu jedem Erstgespräch",
  });

  for (const r of requirements.filter((r) => r.kind === "must").slice(0, 3)) {
    questions.push({
      key: `must_${r.id}`,
      text: `Die Stelle verlangt: "${r.text}". Erzählen Sie von einer Situation, in der Sie das gebraucht haben.`,
  ...
  if (workload) {
    questions.push({
      key: "pressure",
      text: "Erzählen Sie von einer Phase mit hohem Druck. Wie sind Sie damit umgegangen?",
  ...
  questions.push({
    ...
    origin: "Standardfrage; prüft Selbsteinschätzung und Lernen",
  });

  return questions;
}

/** Rückfragen an das Unternehmen, abgeleitet aus dem, was unklar ist. */
function buildCompanyQuestions(
  job: typeof schema.jobs.$inferSelect | null,
  themes: (typeof schema.reviewThemes.$inferSelect)[],
): string[] {
  const out: string[] = [];
  if (job && !job.salaryDisclosed) out.push("In welchem Rahmen bewegt sich das Gehalt für diese Position?");
...
export async function submitCoachingAnswer(
  ...
  if (trimmed.length < 10) return { ok: false, message: "Die Antwort ist zu kurz für eine Rückmeldung." };
  ...
  await withUser(db, user.id, async (tx) => {
    ...
    const hasSituation = /\b(als|damals|einmal|in meinem|bei meinem|während)\b/i.test(trimmed);
    ...
    if (!hasResult) missing.push("Das Ergebnis fehlt. Auch ein grobes zählt.");
    ...
    await tx.insert(schema.coachingFeedback).values({
      ...
      structure: hasSituation && hasAction && hasResult
        ...
        : "Der Aufbau nach Situation, Handlung und Ergebnis ist noch nicht vollständig.",
      concreteEvidence: hasResult
        ? "Es steht ein nachvollziehbares Ergebnis darin."
        : "Ein konkretes Ergebnis fehlt noch.",
      clarity: isLong
        ? "Die Antwort ist ausführlich. Prüf, ob der Kern in den ersten zwei Sätzen steht."
  ...
  return { ok: true, message: "Rückmeldung erstellt." };
}
