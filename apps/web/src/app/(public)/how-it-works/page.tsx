...
export default async function HowItWorksPage() {
  ...
  const steps = [
    {
      title: "Ein Gespraech statt eines Formulars",
      body:
        `${brand.assistantName} fragt nach konkreten Situationen, nicht nach Selbsteinschaetzungen. ` +
        `"Bist du gut im Organisieren?" liefert eine Meinung. "Erzaehl von etwas, das du organisiert ` +
        `hast" liefert etwas, das man belegen und in eine Bewerbung schreiben kann.`,
    },
    {
      title: "Nichts zaehlt, bevor du es bestaetigst",
      body:
        "Aus deinen Antworten entstehen einzelne Aussagen. Jede kannst du bestaetigen, bearbeiten, " +
        "ablehnen oder loeschen. Erst bestaetigte Aussagen fliessen in Empfehlungen und Unterlagen " +
        "ein. Das ist der Unterschied zu einem Chatprofil.",
    },
    {
      title: "Wenige Vorschlaege, jeder begruendet",
      body:
    ...
    },
    {
      ...
      body:
        "Jede pruefbare Aussage in einem erzeugten Dokument haengt an einer bestaetigten Erfahrung. " +
        "Findet sich kein Beleg, wird die Aussage markiert und das Dokument laesst sich nicht " +
        "freigeben. Das ist keine Warnung, sondern eine Sperre.",
    },
    {
      title: "Du entscheidest, was hinausgeht",
      body:
        `${brand.assistantName} bereitet vor, erklaert und fragt nach. Versendet wird nichts ohne ` +
        `deine ausdrueckliche Bestaetigung - und es gibt keinen Massenversand.`,
    },
  ];
    ...
    <Stack gap={6}>
      <header>
        ...
        <p style={{ marginTop: "var(--space-4)", fontSize: "var(--text-lg)", color: "var(--text-secondary)" }}>
          Sechs Punkte, die erklaeren, warum das hier anders ablaeuft als auf einer Jobboerse.
        </p>
      </header>
      ...
      <Card style={{ background: "var(--surface-sunken)", boxShadow: "none" }}>
        <Stack gap={3}>
          <h2 style={{ fontSize: "var(--text-base)" }}>Was hier ausdruecklich nicht passiert</h2>
